...
#include "AP_MotorsMatrix_Custom.h"
#include <AP_Vehicle/AP_Vehicle_Type.h>

extern const AP_HAL::HAL& hal;

// init
void AP_MotorsMatrix_Custom::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    _thrust_from_rc = 0.0;   //AKGL
    _motor_output_from_nuc = 0.0;   //AKGL
    armed(true); //AKGL
    _in_output_func = 0;
    _in_output_2_motors = 0;
    _setting_values = 0;
    _sending_2_motors = 0;
...
bool AP_MotorsMatrix_Custom::init(uint8_t expected_num_motors)
{
...
bool AP_MotorsMatrix_Custom::set_throttle_factor(int8_t motor_num, float throttle_factor)
{
    if ((_active_frame_class != MOTOR_FRAME_SCRIPTING_MATRIX) ) {
        // not the correct class
        return false;
...
void AP_MotorsMatrix_Custom::set_update_rate(uint16_t speed_hz)
{
...
void AP_MotorsMatrix_Custom::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AP_MotorsMatrix_Custom::output_to_motors()
{
    int8_t i;
    _in_output_2_motors++;
    ...
    switch (_spool_state) {
        ...
        case SpoolState::GROUND_IDLE: {   //min output value  
            for (i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
                if (motor_enabled[i]) {
                    _actuator[i] = 0.15;  //AKGL set min value
                }
            }
            break;
        }
        case SpoolState::MANUAL:  // manual mode
            for (i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
                if (motor_enabled[i]) {
                    _actuator[i] = _motor_output_from_nuc;  //AKGL set value from nuc
                }
            }
            break;
        case SpoolState::THROTTLE_UNLIMITED:
            ...
            for (i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
                if (motor_enabled[i]) { //AKGL removed actuator with slew
                    // set_actuator_with_slew(_actuator[i], thr_lin.thrust_to_actuator(_thrust_rpyt_out[i]));
                    _setting_values++;
                    _thrust_rpyt_out[i] = 0.5; //AKGL: Dr. Leobardo, favor de cambiar el valor de _thrust_rpyt_out
                    _actuator[i] = thr_lin.thrust_to_actuator(_thrust_rpyt_out[i]);  //AKGL for motor test
                    // _actuator[i] = thr_lin.thrust_to_actuator(_thrust_rpyt_out[i])*_thrust_from_rc;  //AKGL removed the volt scaling 
                }
            }
            break;
    ...
    for (i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            _thrust_rpyt_out[i] = 0.5; // added
            _actuator[i] = thr_lin.thrust_to_actuator(_thrust_rpyt_out[i]); //added
            _sending_2_motors++;
...
uint32_t AP_MotorsMatrix_Custom::get_motor_mask()
{
...
float AP_MotorsMatrix_Custom::boost_ratio(float boost_value, float normal_value) const
{
...
void AP_MotorsMatrix_Custom::output_armed_stabilizing() //AKGL variable  _thrust_rpyt_out is the one that stores the motors values
                                                        //_roll_in, _pitch_in and other variables can be set from outside
{
//    variables set in control class
//    _roll_in
//    _pitch_in
//    _yaw_in
    ...
    for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            ...
            _thrust_rpyt_out[i] = _roll_in * _roll_factor[i] + _pitch_in * _pitch_factor[i];
...
void AP_MotorsMatrix_Custom::check_for_failed_motor(float throttle_thrust_best_plus_adj)
{
...
void AP_MotorsMatrix_Custom::_output_test_seq(uint8_t motor_seq, int16_t pwm)
{
...
//  pwm value is an actual pwm value that will be output, normally in the range of 1000 ~ 2000
bool AP_MotorsMatrix_Custom::output_test_num(uint8_t output_channel, int16_t pwm)
{
    if (!armed()) {
        return false;
    }
...
// add_motor
void AP_MotorsMatrix_Custom::add_motor_raw(int8_t motor_num, float roll_fac, float pitch_fac, float yaw_fac, uint8_t testing_order, float throttle_factor)
{
    if (initialised_ok()) {
...
void AP_MotorsMatrix_Custom::add_motor(int8_t motor_num, float angle_degrees, float yaw_factor, uint8_t testing_order)
{
    add_motor(motor_num, angle_degrees, angle_degrees, yaw_factor, testing_order);
}

// add_motor using position and prop direction. Roll and Pitch factors can differ (for asymmetrical frames)
void AP_MotorsMatrix_Custom::add_motor(int8_t motor_num, float roll_factor_in_degrees, float pitch_factor_in_degrees, float yaw_factor, uint8_t testing_order)
{
    add_motor_raw(
        motor_num,
...
void AP_MotorsMatrix_Custom::remove_motor(int8_t motor_num)
{
    // ensure valid motor number is provided
    if (motor_num >= 0 && motor_num < AP_MOTORS_MAX_NUM_MOTORS) {
...
void AP_MotorsMatrix_Custom::add_motors(const struct MotorDef *motors, uint8_t num_motors)
{
...
void AP_MotorsMatrix_Custom::add_motors_raw(const struct MotorDefRaw *motors, uint8_t num_motors)
{
    for (uint8_t i=0; i<num_motors; i++) {
...
bool AP_MotorsMatrix_Custom::setup_quad_matrix(motor_frame_type frame_type)
{
    ...
    case MOTOR_FRAME_TYPE_PLUS: {
        ...
        static const AP_MotorsMatrix_Custom::MotorDef motors[] {
    ...
    case MOTOR_FRAME_TYPE_Y4:
        ...
        static const AP_MotorsMatrix_Custom::MotorDefRaw motors[] {
...
bool AP_MotorsMatrix_Custom::setup_hexa_matrix(motor_frame_type frame_type)
{
...
bool AP_MotorsMatrix_Custom::setup_octa_matrix(motor_frame_type frame_type)
{
...
bool AP_MotorsMatrix_Custom::setup_octaquad_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_OCTOROTOR;
...
bool AP_MotorsMatrix_Custom::setup_dodecahexa_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_DODECAROTOR;
...
bool AP_MotorsMatrix_Custom::setup_y6_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_HEXAROTOR;
...
bool AP_MotorsMatrix_Custom::setup_deca_matrix(motor_frame_type frame_type)
{
    _mav_type = MAV_TYPE_DECAROTOR;
...
void AP_MotorsMatrix_Custom::setup_motors(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AP_MotorsMatrix_Custom::normalise_rpy_factors()
{
    float roll_fac = 0.0f;
...
*/
void AP_MotorsMatrix_Custom::thrust_compensation(void)
{
...
*/
void AP_MotorsMatrix_Custom::disable_yaw_torque(void)
{
    for (uint8_t i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        _yaw_factor[i] = 0;
    }
...
float AP_MotorsMatrix_Custom::get_thrust_rpyt_out(uint8_t i) const
{
...
bool AP_MotorsMatrix_Custom::get_factors(uint8_t i, float &roll, float &pitch, float &yaw, float &throttle, uint8_t &testing_order) const
{
...
// singleton instance
AP_MotorsMatrix_Custom *AP_MotorsMatrix_Custom::_singleton;
