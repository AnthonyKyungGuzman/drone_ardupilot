...
/// @class      AP_MotorsMatrix_Custom
class AP_MotorsMatrix_Custom : public AP_MotorsMulticopter {
public:

    /// Constructor
    AP_MotorsMatrix_Custom(uint16_t speed_hz = AP_MOTORS_SPEED_DEFAULT) :
        AP_MotorsMulticopter(speed_hz)
    {
        if (_singleton != nullptr) {
            AP_HAL::panic("AP_MotorsMatrix_Custom must be singleton");
        }
        _singleton = this;
    };

    // get singleton instance
    static AP_MotorsMatrix_Custom *get_singleton() {
        return _singleton;
    }
...
private:
    ...
    static AP_MotorsMatrix_Custom *_singleton;
};
